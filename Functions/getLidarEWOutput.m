...
if input.flag_apply_noise == 1
    ...
    for ind_LOS= 1:length(Y)
           LOS_vec= cell2mat(VFinalTotal_Time_LOS_vec(ind_LOS,:)); % create each beam individually in one variable
            ...
            %Add the noise as originally done.
            LOS_vec(1,:) = awgn(LOS_vec(1,:),noise_U);
            LOS_vec(2,:) = awgn(LOS_vec(2,:),noise_V);
            LOS_vec(3,:) = awgn(LOS_vec(3,:),noise_W);

           for i=1:size(LOS_vec(1,:),2)
               if (i>size(LOS_vec(1,:),2)*1/4) && (i<size(LOS_vec(1,:),2)*1/2)
                   LOS_vec(1,i) = LOS_vec(1,i)*1.5;
               end 
               if (i>size(LOS_vec(1,:),2)*2/3)
                   LOS_vec(1,i) = LOS_vec(1,i)*0.05;
               end
           end
           for i=1:size(LOS_vec(2,:),2)
                if (i>size(LOS_vec(2,:),2)*1/4) && (i<size(LOS_vec(2,:),2)*1/2)
                    %LOS_vec(2,i) = LOS_vec(2,i)*3;
               end 
               if (i>size(LOS_vec(2,:),2)*2/3)
                    LOS_vec(2,i) = LOS_vec(2,i)*0.05;
               end
           end
           for i=1:size(LOS_vec(3,:),2)
                if (i>size(LOS_vec(3,:),2)*1/4) && (i<size(LOS_vec(3,:),2)*1/2)
                    %LOS_vec(3,i) = LOS_vec(3,i);
               end 
               if (i>size(LOS_vec(3,:),2)*2/3)
                    LOS_vec(3,i) = LOS_vec(3,i)*0.05;
               end
           end
