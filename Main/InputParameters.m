...
input.flag_getTurbsimInput    = 0; % Obtain inputs to create constrained windfield with turbsim
...
input.flag_obtain_Con_Pyconturb_python   = 1; % Run pyconturb and obtain constrained wind field through the Python wrapper
...
input.PatternY = {[ 54  54 0 -54 -54]}; % Pattern points Y axis (in meters). Each line of the cell is a pattern
input.PatternZ = {[-54  54 0 -54  54]}; % Pattern points Z axis (in meters). Each line of the cell is a pattern
...
input.PatternNames = {'5P_Rectangular' }; % names of the patterns. Important: number of names should equal number of Y,Z coordinates and both (values and name of the pattern) should follow the same order in its cells

%input.timestep_pat_vec      = {[5 5] [10 4]}; %Time step of the total pattern. Sampling rate of total pattern should be that npoins*timestep_meas<=timestep_pat(s). Add one value for each pattern
%input.timeStep_Measurements = {[0 0] [0]}; %Time step between each single measured point. Add one value for each pattern [s]
...
input.TurbSimExe_path = '/opt/conda/bin/turbsim'; %Full path to the executable of Turbsim. If other platform than windows is used the correct executable is neeeded
...
input.flag_save_pyconturb_input = 1; % obtain PyConTurb input
input.flag_runPyConTurb         = 1; % Run pyconturb
